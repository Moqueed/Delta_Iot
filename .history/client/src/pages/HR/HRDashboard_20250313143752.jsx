      ...
      <Row gutter={[24, 24]} className="w-full h-full p-10" justify="center" align="middle">
        {sections.map((section) => (
          <Col key={section.key} xs={24} sm={12} md={6} lg={6} xl={6} className="flex justify-center">
            <Card
              hoverable
              className={`w-[300px] h-[300px] flex flex-col items-center justify-center shadow-lg rounded-xl p-6 border-8 ${section.borderColor}`}
              onClick={() => navigate(`/${section.key}`)}
            >
              <div className="text-9xl mb-6 text-blue-500">{section.icon}</div>
