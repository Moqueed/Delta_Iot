...
router.post("/upload/:email", upload.single("resume"), async (req, res) => {
  try {
    ...
    // ✅ Update Candidate's resume attachment
    candidate.attachments = filePath;
    await candidate.save();

    // ✅ Fetch entry from ActiveList for this candidate
    let activeEntry = await ActiveList.findOne({ where: { candidate_email_id: email } });
    if (activeEntry) {
      await activeEntry.update({ attachments: filePath });
    }

    // ✅ Fetch entry from Approvals for this candidate
    let approvalEntry = await Approvals.findOne({ where: { candidate_email_id: email } });
    if (approvalEntry) {
      await approvalEntry.update({ attachments: filePath });
    }
