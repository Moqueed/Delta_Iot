...
router.post("/add", async (req, res) => {
  try {
    const { candidate_email_id, ...otherData } = req.body;
    ...
    const newCandidate = await Candidate.create({ candidate_email_id, ...otherData });
    ...
    res.status(201).json({ message: "Candidate added successfully and synced to ActiveList & TotalData!", candidate: newCandidate });
  } catch (error) {
