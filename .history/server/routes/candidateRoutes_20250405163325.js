...
router.post("/add-candidate", async (req, res) => {
  try {
    ...
    const newCandidate = await ActiveList.create({
      candidate_id,
      candidate_name,
      candidate_email_id,
      position,
      department,
      progress_status: progress_status || "Application Received",
      status_date: status_date || new Date(),
      entry_date
    });

    res.status(201).json({ message: "Candidate added and synced to Active List", data: newCandidate });
  } catch (error) {
