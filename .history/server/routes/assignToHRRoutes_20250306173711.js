...
router.get("/", async (req, res) => {
  try {
    const assignments = await AssignToHR.findAll({
      include: [
        { model: HR, attributes: ["id", "name", "email", "contact_number"] }, // ✅ Added HR ID and Name
        { 
          model: Candidate, 
          attributes: ["id", "candidate_name", "candidate_email_id", "position", "contact_number", "attachments"] // ✅ Fixed resume field
        },
      ],
      attributes: ["id", "comments", "attachments", "hr_id", "candidate_id"], // ✅ Added assignment ID, HR ID, Candidate ID
    });
