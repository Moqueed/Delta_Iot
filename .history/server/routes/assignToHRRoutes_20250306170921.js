...
// ✅ Assign Candidate to HR (Auto-sync Resume)
router.post("/assign", async (req, res) => {
  try {
    ...
    // Check if Candidate exists and fetch resume
    const candidate = await Candidate.findByPk(candidate_id);
    if (!candidate) {
      return res.status(404).json({ message: "Candidate not found" });
    }

    // Create assignment with candidate's resume auto-synced
    const newAssignment = await AssignToHR.create({
      hr_id,
      candidate_id,
      attachments: candidate.resume, // Auto-sync resume
      comments,
    });
