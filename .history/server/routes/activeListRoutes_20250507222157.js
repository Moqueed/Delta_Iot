...
const { TotalMasterData, AboutToJoin } = require("../models/TotalData");
...
router.put("/total-master-data/:id", async (req, res) => {
  ...
  try {
    ...
    activeRecord.progress_status = progress_status;
    await activeRecord.save();

    // Check if entry already exists in total_master_data
    ...
    if (existingEntry) {
      return res.status(200).json({ message: "Status updated, entry already exists in master data" });
    }
...
router.put("/about-to-join/:id", async (req, res) => {
  ...
  try {
    ...
    if (existingEntry) {
      return res.status(200).json({ message: "Status updated, entry already exists in about to join" });
    }
